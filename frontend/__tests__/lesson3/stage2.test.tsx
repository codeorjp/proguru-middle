...
describe("Lesson3 Stage2", () => {
  test("build blocks -> eval code -> input text -> submit -> display content", async () => {
    ...
    assignChatInput.nextConnection.connect(display.previousConnection);
    assignChatInput
      .getInput("VARNAME")
      .connection.connect(varText1.outputConnection);
    display.getInput("VARNAME").connection.connect(varText2.outputConnection);
