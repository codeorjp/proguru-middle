...
// NOTICE: テスト内容はLesson4.MessageBoardとほぼ同一
describe("test Lesson3 MessageBoard", () => {
...
describe("only Lesson3", () => {
  describe("types", () => {
    test("SubmitArea props", () => {
      ...
      expect(submitArea.props().types).toMatchObject(["Text"]);
    });
  });

  describe("undefined parameters", () => {
    describe("getPastMessages", () => {
      test("in lesson3 case", () => {
        const store = createStore();
